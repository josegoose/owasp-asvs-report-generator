 ...
 */
    filter('ravg',function () {
        ...
        return function ravg(obj) {
            ...
            avg = values.reduce(function (prev, cur) {
                return prev + cur;
            }, 0) / values.length;
            return Math.floor(avg * 10) / 10;
        };
    }).
    controller('WizardController', ['$scope', 'ravgFilter', function ($scope, ravgFilter) {
        "use strict";
        function getNewReport() {
            ...
            report.rules = report.rules.map(function (rule) {
                var factors = {};
                jQuery.extend(true, factors, ASVS.risk.factors);
                rule.risk = { factors:  factors};
                rule.showEdit = 'hide';
                rule.showPass = 'hide';
                rule.showFail = 'hide';
